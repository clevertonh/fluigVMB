function createDataset(fields, constraints, sortFields) {
    ...
    //dataset.addColumn("DATASET");
   
    
    var nomeDataSetInterno = constraints[0].initialValue;
    
    log.info("NOVO DATASET");
    log.dir(constraints);
    //"VM_SolicitacoesViagens"
    //dataset interno
    ...
    var datasetPrincipal = DatasetFactory.getDataset(nomeDataSetInterno, null, constraintsActive, null);
    
    if(constraints!==null && constraints.length){ //se tiver constraint filtra
        if(constraints[1].constraintType==ConstraintType.MUST) { // implementação somente para o MUST
            for(var a=0;a < datasetPrincipal.rowsCount;a++){
            	...
            	log.info("-----RETORNO CONTRAINT VIAGEM 20:45------");
            	log.dir(constraints);
            // log.info(datasetPrincipal.getValue(a,constraints[0].fieldName));
            	if(constraints[1].initialValue==datasetPrincipal.getValue(a,constraints[1].fieldName)){ 
                    ...
                    log.info("data set filho");
                    log.dir(constraintsFilhos);
                
                    //Busca o dataset
                    var datasetFilhos = DatasetFactory.getDataset(nomeDataSetInterno, null, constraintsFilhos, null);
